...
function isString(value) {
  ...
  if (str === 'string' || value instanceof String === true) {
    return true;
  }
  return false;
}
...
function removeFirstOccurrences(str, value) {
  ...
  const { length } = value;
  const finishIndex = firstIndex + length;
  return str.slice(0, firstIndex) + str.slice(finishIndex);
}
 ...
 *
 * .replace(/[^a-z0-9]/g, '') delete everything exept leters and numbers
 */
function isPalindrome(str) {
  ...
  const firstStr = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const secondStr = reverseStr.toLowerCase().replace(/[^a-z0-9]/g, '');
 ...
 *
 * @param {string} str - The input string.G
 * @example:
...
function extractNameFromTemplate(value) {
  const temStr = value.replace('Hello, ', '');
  return temStr.replace('!', '');
}
